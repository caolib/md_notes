...
<img src="https://gitee.com/clibin/image-bed/raw/master/CopyQ.XBpSgv.png" alt="" style="zoom:;" />
...
![](https://cdn.processon.com/62942804e4b055af8b78ba06?e=1653880340&amp;token=trhI0BY8QfVrIGn9nENop6JAc6l5nZuxhjQ62UfM:UhUsF4fqZnqyZWpsN-hM0r93O4Y=)
...
<img src="https://gitee.com/clibin/image-bed/raw/master/image-20231113184551320.png" style="zoom:80%;" />

> 划分子网案例

<img src="https://gitee.com/clibin/image-bed/raw/master/image-20231113185650959.png" style="zoom:67%;" />

#### 4.2.6 ==无分类编制==的`IP`

<img src="https://gitee.com/clibin/image-bed/raw/master/image-20231113190642722.png" style="zoom:67%;" />

> ==无分类域间路由选择==（`C`lassless `I`nter-`D`omain `R`outing，`CIDR` ）

<img src="https://gitee.com/clibin/image-bed/raw/master/image-20231113190939322.png" style="zoom:67%;" />
...
<img src="https://gitee.com/clibin/image-bed/raw/master/image-20231113193616181.png" style="zoom:67%;" />
...
<img src="https://gitee.com/clibin/image-bed/raw/master/image-20231117222851885.png" style="zoom:67%;" />
...
<img src="https://gitee.com/clibin/image-bed/raw/master/image-20231117223841323.png" style="zoom:67%;" />

#### 3. 路由环路

- 错误配置静态路由有可能导致==路由环路==问题
- 为了==防止IP数据报在环路中永久兜圈==，在IP数据报首部设有==生存时间TTL==，TTL为0时会被丢弃

**路由环路问题产生原因：**

1. 路由配置错误

<img src="https://gitee.com/clibin/image-bed/raw/master/image-20231118125828914.png" style="zoom:67%;" />

2. 聚合了不存在的IP地址

> 需要给不存在的IP地址配置黑洞路由，防止IP数据报转发到其他路由器

<img src="https://gitee.com/clibin/image-bed/raw/master/image-20231118133214440.png" style="zoom:67%;" />

3. 网络故障

> 可以在网络发生故障时==添加一条针对该IP的黑洞路由==，待网络正常后又将其设置为失效状态

<img src="https://gitee.com/clibin/image-bed/raw/master/image-20231118134111043.png" style="zoom: 67%;" />
...
| 静态路由选择==简单、开销小==，但==不能及时适应网络状态（流量、拓扑等）的变化== | 动态路由选择比较==复杂、开销比较大==，但==能较好地适应网络状态的变化== |
...
<img src="https://gitee.com/clibin/image-bed/raw/master/image-20231118150928383.png" style="zoom:67%;" />
...
- RIP使用==跳数==（`Hop Count`）作为度量（`Metric`）来衡量到达目的网络的距离
  ...
  - RIP将==路由器到直连网络的距离定义为1==

  - RIP将路由器到非直连网络的距离定义为==所经过的路由器数加1==
  - RIP允许一条路径最多只能包含15个路由器，==距离等于16时相当于不可达==,因此RIP只适用于==小型互
    联网==

<img src="https://gitee.com/clibin/image-bed/raw/master/image-20231118140619521.png" style="zoom: 50%;" />

2. RIP认为好的路由就是“距离短”的路由，也就是所==通过路由器数量最少的路由==,和传输速率和物理距离等无关

3. RIP的==3个重要特点==

<img src="https://gitee.com/clibin/image-bed/raw/master/image-20231118141136250.png" style="zoom: 67%;" />

4. **==RIP的工作原理==**

   1. 基本流程

   <img src="https://gitee.com/clibin/image-bed/raw/master/image-20231118141750507.png" style="zoom:67%;" />

   2. **==RIP路由条目的更新规则==**

   <img src="https://gitee.com/clibin/image-bed/raw/master/image-20231118142447662.png" style="zoom:80%;" />

   3. ==**坏消息传播的慢**==

      ![](https://gitee.com/clibin/image-bed/raw/master/image-20231118220443883.png)

5. RIP的优缺点

| 优点                                                         | 缺点                                                         |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| 实现==简单==，路由器==开销小==                               | RIP限制了==最大RIP距离为`15`==这就限制了使用RIP的自治系统AS的规模 |
| 如果一个路由器发现了RIP距离更短的路由，那么这种更新信息就传播得很快，即“==好消息传播得快==” | 相邻路由器之间交换的路由信息是路由器中的完整路由表，因而==随着网络规模的扩大，开销也随之增大== |
...
> - `OSPF`是==基于**链路状态**==的，而不像`RIP`是基于距离向量的
...
<img src="https://gitee.com/clibin/image-bed/raw/master/image-20231118223059765.png" style="zoom:80%;" alt='举例-思科路由器的代价计算标准' />
...
<img src="https://gitee.com/clibin/image-bed/raw/master/image-20231118223958856.png" style="zoom:67%;" />

3.  ==**链路状态数据库**==(`Link State Database，LSDB`)

![](https://gitee.com/clibin/image-bed/raw/master/image-20231118224728007.png)

> 使用`OSPF`的各路由器，基于链路状态数据库LSDB进行最短路径优先计算，构建出各自到达其他各路由器的最短路径，即构建各自的路由表(==根据链路状态数据库得出全局带权有向图，使用Dijkstra算法得出个路由器的最短路径==)

![](https://gitee.com/clibin/image-bed/raw/master/image-20231118225157334.png)
...
<img src="https://gitee.com/clibin/image-bed/raw/master/image-20231119152547862.png" style="zoom:67%;" />

> 采用划分区域的方法，虽然使交换信息的种类增多了，同时也使OSPF协议更加复杂了，但这样做能==使每一个区域内部交换路由信息的通信量大大减小，因而使OSPF协议能够用于规模更大的自治系统AS==

<img src="https://gitee.com/clibin/image-bed/raw/master/image-20231119152848527.png" style="zoom:67%;" />

### 4.8  `BGP`

#### 1. 基本概念

> - **边界网关协议**（`Border Gateway Protocol，BGP`）属于==外部网关协议==`EGP`这个类别，用于自治系统`AS`之间的路由选择协议
...
<img src="https://gitee.com/clibin/image-bed/raw/master/image-20231119154933166.png" style="zoom: 80%;" />
...
> - 当BGP发言人相互交换了网络可达性的信息后，各BGP发言人就==根据所采用的策略==，从收到的路由信息中==找出到达各自治系统的较好的路由==，也就是==构造出树形结构且不存在环路的自治系统连通图==

#### 2. 四种报文

<img src="https://gitee.com/clibin/image-bed/raw/master/image-20231119155610676.png"  />

<img src="https://gitee.com/clibin/image-bed/raw/master/image-20231119160021206.png"  />

### 4.9 IPv4数据报的首部格式

![](https://gitee.com/clibin/image-bed/raw/master/image-20231119161128658.png)

|           版本           | 长度为==4==个比特，用来表示IP协议的版本，==通信双方使用的IP协议的版本必须一致==，目前广泛使用的IP协议的版本号为4（即IPv4） |
| :----------------------: | ------------------------------------------------------------ |
|       **首部长度**       | 长度为==4==个比特，该字段的取值==以4字节为单位==，用来表示IPV4数据报的首部长度。<br/>最小取值为二进制的0101，即十进制的5，再乘以4字节单位，==表示IPv4数据报首部只有20字节固定部分==<br />最大取值为二进制的1111，即十进制的15，再乘以4字节单位，==表示IPV4数据报首部包含20字节固定部分和最大40字节可变部分== |
|        **总长度**        | 长度为==16==个比特，该字段的取值==以字节为单位==，用来==表示IPv4数据报的长度==（==首部长度+数据载荷长度==）<br />最大取值为二进制的16个比特1，即十进制的65535（很少传输这么长的IPv4数据报） |
|         **标识**         | 长度为==16==个比特，==属于同一个IPv4数据报的各分片数据报应该具有相同的标识== |
...
|        **片偏移**        | 长度为==16==个比特，该字段的取值，==以8字节为单位==，用来指出分片IPv4数据报的==数据载荷偏移其在原IPv4数据报的位置有多远，只能为整数，否则必须调整前一个分片长度== |
|       **生存时间**       | 长度为==8==个比特，最大取值为二进制的11111111，即十进制的255。该字段的取值最初以秒为单位。因此，IPv4数据报的最大生存时间最初为255秒。路由器转发IPv4数据报时，将其首部中该字段的值减去该数据报在路由器上所耗费的时间，若结果不为0就转发，否则就丢弃<br />生存时间字段==后来改为以“跳数”为单位，路由器收到待转发的IPv4数据报时，将其首部中的该字段的值减1，若结果不为0就转发，否则就丢弃== |
|         **协议**         | 长度为8个比特，用来==指明IPv4数据报的数据载荷是何种协议数据单元PDU==<br />![](https://gitee.com/clibin/image-bed/raw/master/image-20231119165547509.png) |
|      **首部检验和**      | 长度为==16==个比特，用于==检测IPv4数据报在传输过程中其首部是否出现了差错==<br />IPv4数据报==每经过一个路由器==，其首部中的某些字段的值（例如生存时间TTL、标志以及片偏移等）都可能发生变化，因此路由器都要==重新计算一下首部检验和== |
| **源IP地址和目的IP地址** | 长度都为==32==个比特，用来填写发送（接收）IPv4数据报的源（目的）主机的IPv4地址 |

> `MTU`：==最大传送单元，超过最大传送单元的长度的IP数据报必须分片==

![](https://gitee.com/clibin/image-bed/raw/master/image-20231119164911210.png)

![](https://gitee.com/clibin/image-bed/raw/master/image-20231119171554203.png)
...
> - ICMP报文被==封装在IP数据报中作为数据载荷==发送

#### 2. ICMP报文数据类型

- **差错报告报文**：用来==向主机或路由器报告差错情况==

  - **终点不可达**：==当路由器或主机不能交付IP数据报时，就向源点发送终点不可达报文==，具体可再根据ICMP的代码字段细分为目的网络不可达、目的主机不可达、目的协议不可达、目的端口不可达、目的网络未知、目的主机未知等13种

    ![](https://gitee.com/clibin/image-bed/raw/master/image-20231119222854859.png)

  - **源点抑制**：==当路由器或主机由于拥塞而丢弃IP数据报时，就向发送该IP数据报的源点发送源点抑制报文==，使源点知道应当把IP数据报的发送速率放慢

    <img src="https://gitee.com/clibin/image-bed/raw/master/image-20231119222400135.png"  />

  - **超时**：

    - 当路由器收到一个目的IP地址不是自己的IP数据报时，会将其首部中生存时间TTL字段的值减1。若结果不为0，则路由器将该数据报转发出去；==若结果为0，路由器不但要丢弃该数据报，还要向发送该IP数据报的源点发送时间超过（超时）报文==
    - 另外，当终点在预先规定的时间内==未能收到一个数据报的全部数据报分片==时，就把已收到的数据报片都==丢弃==，也会==向源点发送时间超过（超时）报文==

    <img src="https://gitee.com/clibin/image-bed/raw/master/image-20231120194354557.png"  />

  - **参数问题**：当路由器或目的主机收到IP数据报后，根据其首部中的检验和字段的值发现首部在传送过程中出现了误码，就丢弃该数据报，并向发送该数据报的源点发送参数问题报文

    ![](https://gitee.com/clibin/image-bed/raw/master/image-20231120194840526.png)

  - **改变路由（重定向）**：路由器把==改变路由报文==发送给主机，让主机知道下次==应将IP数据报发送给另外的路由器==，这样可以==通过更好的路由到达目的主机==

    ![](https://gitee.com/clibin/image-bed/raw/master/image-20231120195049808.png)

- **以下情况==不应发送ICMP差错报告报文==**：

  - 对==ICMP差错报告报文==不再发送ICMP差错报告报文
  - 对第一个分片的IP数据报片的所有==后续数据报片==都不发送ICMP差错报告报文
  - 对==具有多播地址的IP数据报==都不发送ICMP差错报告报文
  - 对具有==特殊地址==（例如`127.0.0.0`或`0.0.0.0`）的IP数据报不发送CMP差错报告报文。

- **询问报文**：用来==向主机或路由器询问情况==

  - **回送请求和回答**：

    由主机或路由器向一个特定的目的主机或路由器发出。收到此报文的主机或路由器必须给发送该报文的源主机或路由器发送ICMP回送回答报文。这种询问报文==用来测试目的站是否可达以及了解其有关状态==

  - **时间戳请求和回答**：

    用来请求某个主机或路由器回答当前的日期和时间。在ICMP时间戳回答报文中有一个32比特的字段，其中写入的整数代表从1900年1月1日起到当前时刻一共有多少秒。这种询问报文==用来进行时钟同步和测量时间==
...
>
> 在命令行中==使用ping命令用来测试主机或路由器之间的连通性==
>
> - PING是TCP/IP体系结构的==应用层直接使用网际层ICMP==的一个例子，它并不使用运输层的TCP或UDP
> - PING应用所使用的ICMP报文类型为==回送请求和回答==
...
>
> - 在UNIX版本中，具体命令为`traceroute`，其==在运输层使用UDP协议==，==在网络层使用ICMP报文类型只有差错报告报文==
> - 在Windows版本中，具体命令为`tracert`，其==应用层直接使用网际层的ICMP协议，所使用的ICMP报文类型有回送请求和回答报文以及差错报告报文==
>
> **==原理==**：==设置IP数据报TTL为1，经过第一个路由器时，TTL减为0，IP数据报被丢弃，路由器向源主机发送超时差错报告报文，这样就知道了经过的第一个路由器的IP地址，以此类推依次设置TTL为2,3,4，……，直到接收到目的主机的回送请求回答报文==

### 4.11 `VPN`

> ==虚拟专用网==(`Virtual Private Network,VPN`)：**利用公用的==因特网==作为本机构各==专用网之间的通信载体==，这样形成的网络又称为虚拟专用网**
...
> 很显然，全世界可能有很多不同机构的专用网具有相同的专用IP地址，但这并不会引起麻烦，因为==这些专用地址仅在机构内部使用==
>
> **注意**：在因特网中的所有路由器，==对目的地址是专用地址的IP数据报一律不进行转发==，这需要由因特网服务提供者ISP对其拥有的因特网路由器进行设置来实现（需要自己对路由器额外配置）

![](https://gitee.com/clibin/image-bed/raw/master/image-20231120204244180.png)

### 4.12 `NAT`

#### 1. 概述

> - **背景**：尽管因特网采用了无分类编址方法来减缓IPv4地址空间耗尽的速度，但由于==因特网用户数量的急剧增长==，特别是==大量小型办公室==和==家庭网络==接入因特网的需求不断增加，I==Pv4地址空间即将耗尽的危险然仍没有解除==（实际上，因特网号码分配管理局IANN于2011年2月3日宣布，IPv4地址已经分配完毕）
>
> - **==网络地址转换==**（`Network Address Translation，NAT`）技术于1994年被提出，==用来缓解IPv4地址空间即将耗尽的问题==
>   - NAT能==使大量使用内部专用地址的专用网络用户共享少量外部全球地址来访问因特网上的主机和资源==
>   - 这种方法需要在专用网络连接到因特网的路由器上==安装NAT软件==。装有NAT软件的路由器称为==NAT路由器==，它==至少要有一个有效的外部全球地址IPG==。这样，所有使用内部专用地址的主机在和外部因特网通信时，都要==在NAT路由器上将其内部专用地址转换成IPG==

![](https://gitee.com/clibin/image-bed/raw/master/image-20231120210441321.png)

####  2. `NAPT`

> **==网络地址与端口号转换方法==**:将NAT和运输层端口号结合使用，称为网络地址与端口号转换（`Network Address and Port Translation，NAPT`）
>
> - 由于目前绝大多数基于TCP/IP协议栈的网络应用，都使用运输层的传输控制协议TCP或用户数据报协议UDP，==为了更加有效地利用NAT路由器中的全球IP地址==，现在常==将NAT转换和运输层端口号结合使用==
...
![](https://gitee.com/clibin/image-bed/raw/master/image-20231120211722902.png)
...
- 第2~4章依次介绍了计算机网络体系结构中的==物理层==、==数据链路层==和==网络层==，它们共同解决了将主机通过异构网络互联起来所面临的问题，实现了==主机到主机的通信==
- 然而在计算机网络中实际进行==通信的真正实体，是位于通信两端主机中的进程==
- 如何==**为运行在不同主机上的应用进程提供直接的逻辑通信服务**==，就是==运输层的主要任务==，运输层协议又称为端到端协议

![](https://gitee.com/clibin/image-bed/raw/master/image-20231124190055469.png)

### 2. 端口号，复用和分用

#### 1. 运输层端口号

- 运行在计算机上的进程是使用==进程标识符==(`P`rocess `ld`entification,`PID`)来标识的。
  - 然而，因特网上的计算机并不是使用统一的操作系统，而==不同操作系统==(Windows、Linux、.MacOS)
    又==使用不同格式的进程标识符==
  - 为了使运行不同操作系统的计算机的应用进程之间能够基于网络进行通信，就必须==使用统一的方法
    对TCPP体系的应用进程进行标识==
- `TCP/IP`体系结构的运输层使用端口号来标识和区分应用层的不同应用进程。端口号的==长度为6比特，取
  值范围是0~65535==

<img src="https://gitee.com/clibin/image-bed/raw/master/image-20231124191009578.png" style="zoom:100%;" />
...
1. **复用（Multiplexing）：**
   - **定义：** 复用是指==将多个应用程序的数据流合并到一个共享的通信通道上==
...
2. **分用（Demultiplexing）：**
   - **定义：** 分用是指==根据数据流中的标识信息将合并的数据流分发给正确的应用程序==
...
![](https://gitee.com/clibin/image-bed/raw/master/image-20231124192346522.png)

> ==常见协议的分类==

![](https://gitee.com/clibin/image-bed/raw/master/image-20231124192909256.png)

**<a href='https://www.bilibili.com/video/BV1c4411d7jb?t=353.6&p=58'>运输层端口号应用举例</a>**

### 3.  `TCP`和`UDP`的对比

![](https://gitee.com/clibin/image-bed/raw/master/image-20231124195001220.png)

**==注意==**：

- TCP面向连接是==逻辑连接，并非真实物理连接==
...
> TCP为应用程序提供了==流量控制==（`Flow Control`）机制，以解决因==发送方发送数据太快而导致接收方来不及接收，造成接收方的接收缓存溢出==的问题
>
...
![](https://gitee.com/clibin/image-bed/raw/master/image-20231124203621190.png)

![](https://gitee.com/clibin/image-bed/raw/master/image-20231124205122410.png)

2. 例题

   ![](https://gitee.com/clibin/image-bed/raw/master/image-20231124210146804.png)

### 5. TCP的**拥塞(se)控制**

#### 1. 基本概念

<img src="https://gitee.com/clibin/image-bed/raw/master/image-20231124211432668.png" />

#### 2. 4种拥塞控制方法

![](https://gitee.com/clibin/image-bed/raw/master/image-20231124212653543.png)

##### ==1. 慢开始、拥塞避免==

![](https://gitee.com/clibin/image-bed/raw/master/image-20231124213905436.png)
...
![](https://gitee.com/clibin/image-bed/raw/master/image-20231124214535400.png)

2. **==快重传==**

   - **采用快重传算法可以让发送方尽早知道发生了==个别TCP报文段的丢失==**
   - “快重传”是指**使发送方尽快（尽早）进行重传，而不是等重传计时器超时再重传**
     - 这就要求==接收方不要==等待自己发送数据时才进行==捎带确认==，而是==要立即发送确认==，即使收到了==失序的报文段==也要==立即==发出对已收到的报文段的==重复确认==
     - ==发送方==一旦==收到3个连续的重复确认==，就将相应的报文段==立即重传==，而不是等该报文段的重传计时器超时再重传

   ![](https://gitee.com/clibin/image-bed/raw/master/image-20231124215953819.png)

3. ==**快恢复**==

> 与快重传算法配合使用的是**快恢复算法**，==发送方==一旦==收到3个重复确认==，就知道现在==只是丢失了个别的报文段==，于是==不启动慢开始算法，**而是执行快恢复算法**==
>
> - **快恢复算法：**==发送方将慢开始门限ssthresh的值和拥塞窗口cwnd的值**都调整为当前cwnd值的一半**，并开始执行拥塞避免算法==
> - 也有的快恢复实现是把快恢复开始时的cwnd值再增大一些，即cwnd=新ssthresh+3

![](https://gitee.com/clibin/image-bed/raw/master/image-20231125205520540.png)

### 6. TCP超时重传时间的选择

> **==TCP超时重传时间RTO的选择==是TCP==最复杂==的问题之一**
>
> 问题：
>
...
![](https://gitee.com/clibin/image-bed/raw/master/image-20231125211946213.png)

##### 2. RRTd和RTO的计算

![](https://gitee.com/clibin/image-bed/raw/master/image-20231125212210328.png)

#### 发生超时重传时无法测准RTT

![](https://gitee.com/clibin/image-bed/raw/master/image-20231125212838059.png)

> **通过上述两个例子可以看出：当发送方出现超时重传后，收到确认报文段时是无法判断出该确认到底是对原数据报文段的确认还是对重传数据报文段的确认，也就是无法准确测量出RTT，进而无法正确计算RTO**

##### Karn算法及修正

![](https://gitee.com/clibin/image-bed/raw/master/image-20231125213341384.png)

##### 总结

![](https://gitee.com/clibin/image-bed/raw/master/image-20231125213649193.png)

### 7.TCP可靠传输的实现

- **TCP的窗口以字节为单位**

![](https://gitee.com/clibin/image-bed/raw/master/image-20231126212908308.png)

- 发送方
  - 发送窗口内的已发送数据如果迟迟未收到确认，会发生==超时重传==
  - 只有处于发送窗口内的数据才能发送

![](https://gitee.com/clibin/image-bed/raw/master/image-20231126213314315.png)

- 接收方
  - 接收方==**只能对按序收到的数据中的最高序号给出累计确认**==，3次重复确认会导致发送方==**快重传**==
  - **序号落入接收窗口内的数据是允许接收的数据**

![](https://gitee.com/clibin/image-bed/raw/master/image-20231126213800870.png)

- **==总结==**

![](https://gitee.com/clibin/image-bed/raw/master/image-20231126214833403.png)

### 8. TCP的运输连接管理

#### 1. TCP连接的建立

![](https://gitee.com/clibin/image-bed/raw/master/image-20231203192027726.png)

> TCP双方连接的建立要解决的三个问题

![](https://gitee.com/clibin/image-bed/raw/master/image-20231203192339624.png)

#### 2. 三报文握手

![](https://gitee.com/clibin/image-bed/raw/master/image-20231203193842118 .png)

> **思考：第三次确认==是否多余==，==能不能两报文握手？==**
>
> 答案：不能，如下图所示

![](https://gitee.com/clibin/image-bed/raw/master/image-20231203194753830.png)

#### 3. 四报文挥手

![](https://gitee.com/clibin/image-bed/raw/master/image-20231203195725711.png)

> **思考：为什么客户端发送完最后一个确认报文段后不立刻关闭而是等待2个MSL时间后才关闭？**
>
> 答案：如图所示

![](https://gitee.com/clibin/image-bed/raw/master/image-20231203200428799.png)

> ==TCP保活计时器的作用==

![](https://gitee.com/clibin/image-bed/raw/master/image-20231203201004403.png)
...
![](https://gitee.com/clibin/image-bed/raw/master/image-20231203210526922.png)

![](https://gitee.com/clibin/image-bed/raw/master/image-20231203210406504.png)

#### 2. `P2P`方式

![](https://gitee.com/clibin/image-bed/raw/master/image-20231203210957388.png)

![](https://gitee.com/clibin/image-bed/raw/master/image-20231203210908603.png)
...
![](https://gitee.com/clibin/image-bed/raw/master/image-20231203212822735.png)

#### 3. `DHCP`中继代理

![](https://gitee.com/clibin/image-bed/raw/master/image-20231203213528827.png)
...
![](https://gitee.com/clibin/image-bed/raw/master/image-20231204215706663.png)

![](https://gitee.com/clibin/image-bed/raw/master/image-20231204215946064.png)
...
![](https://gitee.com/clibin/image-bed/raw/master/image-20231204220316573.png)

##### 2. 不同国家的域名

![](https://gitee.com/clibin/image-bed/raw/master/image-20231204220844413.png)

##### 3. 因特网的域名空间

![](https://gitee.com/clibin/image-bed/raw/master/image-20231204221139510.png)

#### 3. 域名服务器

![](https://gitee.com/clibin/image-bed/raw/master/image-20231204221502337.png)

#### 4. 域名解析

##### 1. 递归查询

![](https://gitee.com/clibin/image-bed/raw/master/image-20231204221840693.png)

##### 2. 迭代查询

![](https://gitee.com/clibin/image-bed/raw/master/image-20231204222346748.png)

##### 3. 高速缓存

![](https://gitee.com/clibin/image-bed/raw/master/image-20231204222807218.png)

![](https://gitee.com/clibin/image-bed/raw/master/image-20231204222817296.png)

### 5. `FTP`

> **文件传送协议**（`F`ile `T`ransfer `P`rotocol，`FTP`）

![](https://gitee.com/clibin/image-bed/raw/master/image-20231204223526085.png)

> **基本工作原理**

1. 主动模式

![](https://gitee.com/clibin/image-bed/raw/master/image-20231204224108197.png)

2. 被动模式

![](https://gitee.com/clibin/image-bed/raw/master/image-20231204224211222.png)

### 6. 电子邮件

#### 1. `E-mail`

![](https://gitee.com/clibin/image-bed/raw/master/image-20231205090041032.png)

#### 2. 电子邮件系统的组成

![](https://gitee.com/clibin/image-bed/raw/master/image-20231205090359331.png)

#### 3. 邮件发送和接收过程

![](https://gitee.com/clibin/image-bed/raw/master/image-20231205090654917.png)

#### 4. `SMTP`

> 简单邮件传送协议（Simple Mail Transfer Protocol，`SMTP`）的基本工作过程

![](https://gitee.com/clibin/image-bed/raw/master/image-20231205091401858.png)

#### 5. 电子邮件的信息格式

![](https://gitee.com/clibin/image-bed/raw/master/image-20231205091646693.png)

#### 6. `MIME`

> **多用途因特网邮件扩展** (Multipurpose Internet Mail Extensions，`MIME`)

![](https://gitee.com/clibin/image-bed/raw/master/image-20231205091958949.png)

> **`MIME`对邮件内容中的非ASCII字符转换成ASCII字符**

![](https://gitee.com/clibin/image-bed/raw/master/image-20231205092205673.png)
...
![](https://gitee.com/clibin/image-bed/raw/master/image-20231205092629754.png)

#### 8. 基于万维网的电子邮件

![](https://gitee.com/clibin/image-bed/raw/master/image-20231205093140031.png)

### 7. 万维网

#### 1. 概述

> - **==万维网==**（World Wide Web，`WWW`）==并非某种特殊的计算机网络==。它是一个大规模的、联机式的信息储藏所，是==运行在因特网上的一个分布式应用==
...
![image-20231205094301978](https://gitee.com/clibin/image-bed/raw/master/image-20231205094301978.png)

#### 3. 万维网文档

![](https://gitee.com/clibin/image-bed/raw/master/image-20231205094813054.png)
...
![](https://gitee.com/clibin/image-bed/raw/master/image-20231205095412658.png)

##### 2. 报文格式

- ==请求报文格式==

![](https://gitee.com/clibin/image-bed/raw/master/image-20231205095830137.png)

- ==响应报文格式==

![](https://gitee.com/clibin/image-bed/raw/master/image-20231205100113338.png)
...
![](https://gitee.com/clibin/image-bed/raw/master/image-20231205103323858.png)

#### 6. 万维网缓存与代理服务器

![](https://gitee.com/clibin/image-bed/raw/master/image-20231205103638305.png)
